...
void GraphAlign::outPut(){
  int firstNum = result_nodes_.size();
  int secondNum, thirdNum;
  for(size_t i = 0; i<firstNum; i++){
    secondNum = result_nodes_[i].size();
    for(size_t j=0; j<secondNum; j++){
      thirdNum = result_nodes_[i][j].size();
      std::cout << "(" <<firstNum<< ", "<< secondNum << ", " << thirdNum <<")"<<std::endl;
      for(size_t k=0; k<thirdNum; k++){
        GraphResultNodePtr correspond = result_nodes_[i][j][k];
        std::cout << "first index : " << correspond->getFirstIdx()<<std::endl;
        std::cout << "second index : " <<correspond->getSecondIdx()<<std::endl;
        std::cout << "result score : " << correspond->getResultScore()<<std::endl;
